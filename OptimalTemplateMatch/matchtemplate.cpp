...
void MatchTemplate::multimatch(Mat srcImg, Mat templateImg, Mat &matchResult, int a)
{
    vector<Point> P;
    ...
    matchTempl.GetAllMinLoc(srcImg, templateImg, 0.1, Scalar(0,0,0), &P);
    qDebug() << "P value: " << P[0].x << P[0].y << endl;
    for (int k = 0; k < P.size(); k++)
    {
        Point loc = P[k];
        Mat result_image = srcImg.clone();
        rectangle(srcImg, Rect(loc.x, loc.y, templateImg.cols, templateImg.rows), Scalar(0, 0, 255), 2, 8);
        matchResult = srcImg.clone();
    }
...
//    outerCircle = cv::Mat::zeros(srcImg.size(), CV_8UC1);
//    innerCircle = cv::Mat::zeros(srcImg.size(), CV_8UC1);
//    roi = cv::Mat::zeros(srcImg.size(), CV_8UC1);

//    int outerRadius = templateImg.cols/2;
//    int innerRadius = 500;
//    circle(innerCircle, P[0], innerRadius, CV_RGB(255, 255, 255), -1);
//    circle(outerCircle, P[0], outerRadius, CV_RGB(255, 255, 255), -1);

//    cv::subtract(outerCircle, innerCircle, roi);
//    namedWindow("roi iamge", WINDOW_NORMAL);
//    imshow("roi window", roi);
//    waitKey(0);
}
...
//        cv::imshow("resultMinus",resultImg);
}
void MatchTemplate::matchTemplateBatch(std::string srcImgpath, cv::Mat tempImg, std::string resultpath, cv::Mat matchResult )
{
    ...
    for(int i = 0; i < fileNames.size(); i++)
    {
        ...
        MatchTemplate::multimatch(frames, tempImg, matchResult, 0);
...
void MatchTemplate::ExtarctMatchTemplateCenter(const Mat &srcImg, const Mat &templateImg, vector<cv::Point> &circleCenter)
{
    ...
    matchTempl.GetAllMinLoc(srcImg, templateImg, 0.1, Scalar(0,0,0), &pointsVec);

    for (int i = 0; i < pointsVec.size(); i++)
    {
        circleCenter.push_back(Point(pointsVec[i].x + templateImg.cols/2, pointsVec[i].y + templateImg.rows/2));
    }
}
...
//    cv::imshow("binaryImg",binaryResultImg);
}
...
//    img = imread("/mnt/hgfs/VMsharedfiles/SavedImages/异色/0453-0003-14_异色_0.jpg");//从指定路径加载图像
//    img_temp = imread("/mnt/hgfs/VMsharedfiles/INAAT/SavedImages/model3.jpg");
